   ...
   public function GetAccountSettings(Request $request){
    ...
    public function GetTicketListCustomer(Request $request) {
        ...
        try {
            ...
            ->limit(30)
            ->get(["employees.*","services.id as service_id","master_service_status.*","master_service_priority.*","services.*","contracts.*","customers.*","master_issue_type.*","master_service_type.*"]);
    ...
    public function GetServiceTicketByIdCustomer(Request $request){
        ...
        try {
            ...
            ->first(["services.id as service_id","master_service_status.*","master_service_priority.*","services.*","contracts.*","customers.*","master_issue_type.*","master_service_type.*"]);
            
            
           // $service->status=$this->GetStatusList();  
           if($service){
                  $service->service_date =  Carbon::parse($service->service_date)->format("d-M-Y H:s a");
    ...
    public function GetServiceTicketById(Request $request){
        ...
        try {
            ...
            ->where("services.id",$id)
            ->first(["master_site_area.*","customer_sites.*","services.id as service_id","master_service_status.*","master_service_priority.*","services.*","contracts.*","customers.*","master_issue_type.*","master_service_type.*"]);
    ...
    public function GetServiceHistory(Request $request){
        ...
        try {
            ...
            ->where("service_action_history.service_id",$serviceId)->orderBy('service_action_history.created_at', 'DESC')->get();
    ...
    public function GetTickets(Request $request) {
        ...
        try {
            ...
            ->where("service_status","$ticket_status")
            ->get(["employees.*","services.id as service_id","master_service_status.*","master_service_priority.*","services.*","contracts.*","customers.*","master_issue_type.*","master_service_type.*"]);
            foreach($services as $service){
                $service->serviceDate =  Carbon::parse($service->service_date)->format("d-M-Y H:s a");
    ...
    public function EngineerDashboard(Request $request){
        ...
        try{
            ...
            $new_ticket = Service::where("service_status","2")->where("assigned_to",$userId)->get()->count();
            $open_ticket = Service::where("service_status","3")->where("assigned_to",$userId)->get()->count();
            $pending_ticket = Service::where("service_status","4")->where("assigned_to",$userId)->get()->count();
            $resolved_ticket = Service::where("service_status","5")->where("assigned_to",$userId)->get()->count();
            $closed_ticket = Service::where("service_status","6")->where("assigned_to",$userId)->get()->count();
     ...
     public function userDetail($email,$role) {
        ...
        if($email != "" && $role!="") {
            if($role == 1){
                ...
                $user           =       User::join("customers","customers.CST_ID","users.id")->where("email", $email)->first();
