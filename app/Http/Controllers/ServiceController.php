...
class ServiceController extends Controller
{
    ...
    public function getCustomer($contractId){
        ...
        try {
            ...
            $Customer = Customer::where("CST_ID",$CustomerId)->first();
        } catch(Illuminate\Database\QueryException $ex){
    ...
    public function GetServiceHistory(Request $request){
        ...
        try {
            ...
            ->get(["service_action_history.created_at as historyDate","service_action_history.*",
                "master_service_action_reason.*","master_service_status.*"]);
    ...
    public function GetCustomerId($CustomerId){
        $Customer= array();    
        try {
           $Customer = Customer::where("CST_ID",$CustomerId)
            ->join("users","users.id","customers.CST_ID")->first();
    ...
    public function GetServiceCall(Request $request){
            ...
            return $services;
                ...
                ->orderby("services.updated_at","DESC")
                ->whereBetween(DB::raw('DATE_FORMAT(services.service_date, "%Y-%m-%d")'),[$todate,$fromdate])
                ->where("services.service_status",$status)
                ->get(["customers.*","services.id as service_id","users.name as assignedName","users.name","master_service_type.*","master_issue_type.*","services.*","master_service_status.*","master_service_priority.*"]);
            foreach($services as $service){
                $service->View = "/service/view-service?id=".$service->service_id;
            }
    ...
    public function GetServiceCallById(Request $request){
        ...
        try {
            $id = $request->id;
            $service = Service::join("customers","customers.CST_ID","services.customer_id")
            ->join("master_service_status","master_service_status.Status_Id","services.service_status")
            ->join("master_service_priority","master_service_priority.id","services.service_priority")
            ->join("master_issue_type","master_issue_type.id","services.issue_type")
            ->join("master_service_type","master_service_type.id","services.service_type")
    ...
    public function AddServiceCall(Request $request){

        $validator  =   Validator::make($request->all(),
            [
                "Service_No" => "required",
                "Call_Date"=>"required",
                'Call_Customer_Id' => "required",
                'Call_Contact_Name' => "required",
                'Call_Contact_Number' => "required",
                'Call_Issue_Type' => "required",
                'Call_Service_Type' => "required",
                'Call_Priority' =>"required"
            ]
        );
        ...
        try {
            ...
            if($check == 0){
                $service = Service::create([
                    'service_no'=>$request->Service_No,
                    'service_date'=>date('Y-m-d H:i:s', strtotime($request->Call_Date)),
                    'customer_id'=>$request->Call_Customer_Id,
                    'contract_id'=>$request->Call_Contract_Id,
                    'contact_person'=>$request->Call_Contact_Name,
                    'contact_number1'=>$request->Call_Contact_Number,
                    'contact_number2'=>$request->Call_Contact_AltNumber,
                    'contact_email'=>$request->Call_Contact_Email,
                    'site_location'=>$request->Call_Service_Location,
                    'site_google_link'=>$request->Call_Googledirectionlink,
                    'issue_type'=>$request->Call_Issue_Type,
                    'service_type'=>$request->Call_Service_Type,
                            ...
                            try{
                                $product =  ServiceUnderProduct::create([
                                  ...
                                  if($product){
                                    ...
                                    return response()->json(['success' => true, 'message'=> 'Service Created, Failed to add product.']); 
                                  }
                            }catch(Exception $e){
                                return response()->json(['success' => true, 'message'=> 'Service Created']); 
                            }
